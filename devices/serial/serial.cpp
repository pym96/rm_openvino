...
ReceiveData Serial::getData()
{
    ...
    ReceiveData data{read_yaw.f, read_pitch.f, last_shoot_speed,detect_color};
...
bool Serial::readSerial()
{
    ...
    */
    ...
    // 串口句柄锁
    {
        ...
        if (read(fd, read_buffer_ + 1, 14) != 14) {
            // 读取剩余内容失败
            return false;
        }
    ...
    // 赋值, 数据互斥锁
    {
        ...
        color = this->read_buffer_[13]; // Get current color

        if(color == 0x00){  
            fmt::print(fg(fmt::color::blue),"Current detected color is blue\n");
            dk.setColor(0);
            fmt::print(fg(fmt::color::blue),"{}\n",dk.detectRed());
        }else{
            fmt::print(fg(fmt::color::red),"Current detected color is red\n");
            dk.setColor(1);
            fmt::print(fg(fmt::color::red),"{}\n",dk.detectRed());
        }

        detect_color = read_buffer_[13];
        fmt::print(fg(fmt::color::light_green),"{}",detect_color);
